...
class Extractor {
    ...
    private static void check(State s, int x, int y, int dx, int dy, Counter count) {
        ...
        if (!s.inBound(posX, posY))
            ...
            // left out of bound
            checkOnly(s, x, y, dx, dy, count);
        else if (s.board.is(who, posX, posY) && s.board.is(who, negX, negY))
            // left right all me
            ...
            checkDisconnection(s, x, y, dx, dy, count);
                 ...
                 (s.board.is(who, posX, posY)  && s.board.is(!who, negX, negY))) {
            // one side enemy other side me
            checkOnly(s, x, y, dx, dy, count);
            checkOnly(s, x, y, -dx, -dy, count); }
        else if (s.board.isEmpty(posX, posY) && s.board.isEmpty(negX, negY))
            // both sides empty
    ...
    private static void checkOneSide(State s, int x, int y, int dx, int dy, Counter count) {
        ...
        if (s.inBound(llx, lly) && s.board.is(who, llx, lly)) {
            if (s.inBound(rrx, rry) && s.board.is(who, rrx, rry)) {
                // o-o-o
                if ((!s.inBound(llx - dx, lly - dy) || !s.board.is(who, llx - dx, lly - dy)) &&
                    (!s.inBound(rrx + dx, rry + dy) || !s.board.is(who, rrx + dx, rry + dy))) {
        ...
        } else if (s.inBound(rrx, rry) && s.board.is(who, rrx, rry)) {
            // --o-o
            String right = rightHelper(s, rrx - dx, rry - dy, dx, dy);
    ...
    private static String leftHelper(State s, int x, int y, int dx, int dy) {
        ...
        boolean who = s.board.isBlack(nx, ny);
        if (who) {
        ...
        while (s.inBound(nx, ny)) {
            if (s.board.is(who, nx, ny)) {
                feature = ONE + feature;
                nx -= dx; ny -= dy;
            } else if (s.board.isEmpty(nx, ny) && s.inBound(nx - dx, ny - dy) &&
                s.board.is(who, nx - dx, ny - dy)) {
                feature = EMPTY + feature;
                nx -= dx; ny -= dy;
            } else {
                if (s.board.isEmpty(nx, ny))
                    feature = EMPTY + feature;
                else
    ...
    private static void checkOnly(State s, int x, int y, int dx, int dy, Counter count) {
        ...
        if (s.inBound(nx, ny) && !s.board.isEmpty(nx, ny)) {
            right = rightHelper(s, x, y, dx, dy);
        } else if (s.inBound(nx + dx, ny + dy) && s.board.is(who, nx + dx, ny + dy)) {
